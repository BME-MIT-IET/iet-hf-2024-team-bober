...
public class Window extends Observer{
    ...
    public Window(Controller controller, Game game){
        ...
        attack.addMenuListener(new ViewMenuListener(()->{
           ...
           ArrayList<Virologist> listWithoutCurrentPlayer = v.getField().GetVirologists();
           listWithoutCurrentPlayer.remove(v);
            if(listWithoutCurrentPlayer.size() == 0)
            {
                JMenuItem item = new JMenuItem("Noone to attack!");
                attack.add(item);
            }
            else
            {
                for (Virologist vir : listWithoutCurrentPlayer) {
    ...
    //FELSZERELÉSEK FRISSÍTÉSE
        ...
        for (int i = 0; i < 3; i++){
            if (equipment.size() > i) {
                ...
                try {
                    equipmentSlotIcon = ImageIO.read(Objects.requireNonNull(getClass().getResourceAsStream("/virologist/textures/itemSlot.png")));
                    equipmentSlotIcon = equipmentSlotIcon.getScaledInstance(50, 50, Image.SCALE_SMOOTH);
                } catch (IOException ignored) {}
            }
    ...
    //KÖRVÉGE GOMB BEÁLLÍTÁSA
        ...
        try {
            endButtonIcon = ImageIO.read(Objects.requireNonNull(getClass().getResourceAsStream("/virologist/textures/endButton.png")));
    ...
    //SZÖVEGBUBORÉK BEÁKKÍTÁSA
        ...
        try{
            actionBubbleIMG = ImageIO.read(Objects.requireNonNull(getClass().getResourceAsStream("/virologist/textures/bubble.png")));
    ...
    //HÁTTÉRKÉP BEÁLLÍTÁSA
        ...
        try {
            backGroundIMG = ImageIO.read(Objects.requireNonNull(getClass().getResourceAsStream("/virologist/textures/Field.png")));
            backGroundIMG = backGroundIMG.getScaledInstance(600, 600, Image.SCALE_SMOOTH);
